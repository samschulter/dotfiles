...
We previously had: =ivy=, =counsel=, =swiper=, =all-the-icons-ivy=, and =ivy-rich=.  After watching the video of Mike Zamansky (https://www.youtube.com/watch?v=5ffb2at2d7w), I'm trying =vertico= with =orderless=, =consult=, =embark= and =marginalia=.
...
- What is missing now that I had earlier? Or what does not work?
  ...
  - Something is now wrong with org-journal: "Overwriting value of ‘org-tab-first-hook’ by aliasing to ‘org-cycle-tab-first-hook’"?
  - The search is not so great yet I guess? I don't have the "search-at-point" function. Also, the search would not open org headings. What do other vertico users do for search? Do they just use standard isearch?
